    ...
    def update_room(self, room_number, room_type=None, price_per_night=None, size_room=None, has_view=None):
        ...
        if my_room:
            self.__my_session.query(Rooms).filter_by(room_number=room_number).update({
                ...
                "has_view": has_view or my_room.has_view,
            })
            self.__my_session.commit()
...
if __name__ == "__main__":
    ...
    model.create_room(121212, "double", 100, 4, has_view=True)
    model.update_room(121212, "double", 100, 4, has_view=False)

    for room in model.read_room():
        print(room)

    model.delete_room(121212)
